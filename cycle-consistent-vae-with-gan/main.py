...
def encoder(image,kernel,stride,class_dim,style_dim,is_training,name):
    with tf.variable_scope(name,reuse=tf.AUTO_REUSE):
        ...
        style_miu = fc(fc1,style_dim,is_training,layers.batch_norm,leaky_rectify,'style_miu')
        style_sigma = fc(fc1,style_dim,is_training,layers.batch_norm,leaky_rectify,'style_sigma')
        return class_vector, style_miu, style_sigma, sp[1], sp[2], sp[3]
...
def cycle_consistent_vae_with_gan(image1,image2,image3,kernel,stride,class_dim,style_dim,is_training,reconstruct_coef,kl_coef,generator_coef,discriminator_coef,name):
    ...
    with tf.variable_scope(name,reuse=tf.AUTO_REUSE):
        class_vector_1, style_miu_1, style_sigma_1, w, h, c = encoder(image1,kernel,stride,class_dim,style_dim,is_training,'encoder')
        class_vector_2, style_miu_2, style_sigma_2, _, _, _ = encoder(image2,kernel,stride,class_dim,style_dim,is_training,'encoder')
        class_vector_3, style_miu_3, style_sigma_3, _, _, _ = encoder(image3,kernel,stride,class_dim,style_dim,is_training,'encoder')
        w,h,c = int(w), int(h), int(c)

        style_vector_1 = style_miu_1 + style_sigma_1 * tf.random_normal(tf.shape(style_miu_1), 0, 1, dtype=tf.float32)
        style_vector_2 = style_miu_2 + style_sigma_2 * tf.random_normal(tf.shape(style_miu_2), 0, 1, dtype=tf.float32)
        style_vector_3 = style_miu_3 + style_sigma_3 * tf.random_normal(tf.shape(style_miu_3), 0, 1, dtype=tf.float32)
        ...
        kl_loss_1 = tf.reduce_mean(0.5 * tf.reduce_sum(tf.square(style_miu_1) + tf.square(style_sigma_1) - tf.log(1e-8 + tf.square(style_sigma_1)) - 1, 1))
        kl_loss_2 = tf.reduce_mean(0.5 * tf.reduce_sum(tf.square(style_miu_2) + tf.square(style_sigma_2) - tf.log(1e-8 + tf.square(style_sigma_2)) - 1, 1))
        kl_loss = kl_coef * (kl_loss_1 + kl_loss_2)
        ...
        forward_loss = reconstruct_loss + kl_loss

        # reverse
        sample = tf.random_normal(tf.shape(style_miu_3), 0, 1, dtype=tf.float32)
        ...
        class_vector_reverse_1, style_miu_reverse_1, style_sigma_reverse_1, _, _, _ = encoder(image1_reverse_reconstruct,kernel,stride,class_dim,style_dim,is_training,'encoder')
        class_vector_reverse_3, style_miu_reverse_3, style_sigma_reverse_3, _, _, _ = encoder(image3_reverse_reconstruct,kernel,stride,class_dim,style_dim,is_training,'encoder')

        reverse_loss_1 = tf.reduce_mean(tf.reduce_sum(tf.abs(sample - style_miu_reverse_1),1))
        reverse_loss_3 = tf.reduce_mean(tf.reduce_sum(tf.abs(sample - style_miu_reverse_3),1))
        ...
        return forward_loss, kl_loss, reconstruct_loss, reverse_loss, generator_loss, discriminator_loss, image1_forward_reconstruct, image2_forward_reconstruct
...
def main():
    ...
    forward_loss, kl_loss, reconstruct_loss, reverse_loss, generator_loss, discriminator_loss, image1_forward_reconstruct, image2_forward_reconstruct = cycle_consistent_vae_with_gan(
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            for idx in range(0, imageNum, batch_size):
                ...
                # forward
                _,_,_forward_loss,_kl_loss,_reconstruct_loss,_generator_loss = sess.run([forward_train,generator_train,forward_loss,kl_loss,reconstruct_loss,generator_loss],feed_dict=feed_dict_training)
            ...
            print('epoch: %d\nforward_loss: %f, kl_loss: %f, reconstruct_loss: %f\nreverse_loss: %f, generator_loss: %f, discriminator_loss: %f\n' % (epoch, get_mean(forward_losses), get_mean(kl_losses), get_mean(reconstruct_losses), get_mean(reverse_losses), get_mean(generator_losses), get_mean(discriminator_losses)))
