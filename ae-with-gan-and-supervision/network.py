...
def cycle_consistent_vae_with_gan(image1,image2,kernel,stride,class_dim,is_training,reconstruct_coef,generator_coef,discriminator_coef,name):
    ...
    with tf.variable_scope(name,reuse=tf.AUTO_REUSE):
        class_vector_1, w, h, c = encoder(image1,kernel,stride,class_dim,is_training,'encoder')
        w,h,c = int(w), int(h), int(c)

        # forward
        image1_forward_reconstruct = decoder(class_vector_1,w,h,c,kernel,stride,is_training,'decoder')
        ...
        return forward_loss, reconstruct_loss, generator_loss, discriminator_loss, image1_forward_reconstruct, class_vector_1