...
def init():
    ...
    parser.add_argument('--kl_coef', type=float, default=0.01, help="kl coef")
...
def plot(image1_plot, image1_reconstruct, epoch):
...
def cycle_consistent_vae_with_gan(image1,kernel,stride,class_dim,style_dim,is_training,reconstruct_coef,kl_coef,generator_coef,discriminator_coef,name):
...
def main():
    ...
    forward_loss, reconstruct_loss, generator_loss, discriminator_loss, image1_forward_reconstruct = cycle_consistent_vae_with_gan(
                                                                                                      image1,kernel,stride,class_dim,style_dim,is_training,
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            for idx in range(0, imageNum, batch_size):
                ...
                feed_dict_training = {image1:image1_batch,is_training:True}
            ...
            feed_dict_not_training = {image1:image1_plot,is_training:False}
            image1_reconstruct = sess.run(image1_forward_reconstruct,feed_dict=feed_dict_not_training)
            plot(image1_plot, image1_reconstruct, epoch)
