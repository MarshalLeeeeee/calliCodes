...
def init():
    ...
    parser.add_argument('--forward_reconstruct_coef', type=float, default=1.0, help="forward reconstruct coef")
    parser.add_argument('--reverse_reconstruct_coef', type=float, default=1.0, help="reverse reconstruct coef")
...
def find_truth(image1,imageTrue):
	...
	for fname in image1:
		flabel = int(fname.split('/')[-1].split('.')[0].split('-')[0])
		image2.append(imageTrue[flabel])
	return image2
...
def plot(image1_plot, image3_plot, image_class1_style3, image_class3_style1, image1_truth, image3_truth, epoch):
    num, w, h, c = image1_plot.shape[0], image1_plot.shape[1], image1_plot.shape[2], image1_plot.shape[3]
    img = Image.new('L',(w*6,h*num))
    for i in range(num):
        ...
        img.paste(Image.fromarray(np.squeeze((image1_truth[i]*255).astype(np.uint8))),(64*4,64*i))
        img.paste(Image.fromarray(np.squeeze((image3_truth[i]*255).astype(np.uint8))),(64*5,64*i))
...
def cycle_consistent_vae_with_gan(image1,image2,image3,image4,image5,kernel,stride,class_dim,style_dim,is_training,forward_reconstruct_coef,reverse_reconstruct_coef,generator_coef,discriminator_coef,name):
    ...
    with tf.variable_scope(name,reuse=tf.AUTO_REUSE):
        ...
        forward_reconstruct_loss_1 = tf.reduce_mean(tf.reduce_sum(tf.abs(image1-image1_forward_reconstruct),[1,2,3]))
        forward_reconstruct_loss_2 = tf.reduce_mean(tf.reduce_sum(tf.abs(image2-image2_forward_reconstruct),[1,2,3]))
        forward_reconstruct_loss = forward_reconstruct_coef * (forward_reconstruct_loss_1 + forward_reconstruct_loss_2)
        ...
        reverse_reconstruct_loss_1 = tf.reduce_mean(tf.reduce_sum(tf.abs(image4-image1_reverse_reconstruct),[1,2,3]))
        reverse_reconstruct_loss_2 = tf.reduce_mean(tf.reduce_sum(tf.abs(image5-image3_reverse_reconstruct),[1,2,3]))
        reverse_reconstruct_loss = reverse_reconstruct_coef * (reverse_reconstruct_loss_1 + reverse_reconstruct_loss_2)
        reconstruct_loss = forward_reconstruct_loss + reverse_reconstruct_loss
        ...
        image4_pred_true = discriminator(image4,kernel,stride,is_training,'discriminator')
        image5_pred_true = discriminator(image5,kernel,stride,is_training,'discriminator')
        ...
        discriminator_loss = -discriminator_coef * (discriminator_true_1 + discriminator_true_2 + discriminator_true_3  + discriminator_true_4 + discriminator_true_5 + discriminator_forward_fake_1 + discriminator_forward_fake_2 + discriminator_reverse_fake_1 + discriminator_reverse_fake_3)

        return reconstruct_loss, forward_reconstruct_loss, reverse_reconstruct_loss, reverse_loss, generator_loss, discriminator_loss, image1_forward_reconstruct, image2_forward_reconstruct, image1_reverse_reconstruct, image3_reverse_reconstruct
...
def main():
    ...
    forward_reconstruct_coef = parser.forward_reconstruct_coef
    reverse_reconstruct_coef = parser.reverse_reconstruct_coef
    ...
    reconstruct_loss, forward_reconstruct_loss, reverse_reconstruct_loss, reverse_loss, generator_loss, discriminator_loss, \
    image1_forward_reconstruct, image2_forward_reconstruct, image1_reverse_reconstruct, image3_reverse_reconstruct = cycle_consistent_vae_with_gan(
                                                                                                                     image1,image2,image3,image4,image5,
                                                                                                                     kernel,stride,class_dim,style_dim,is_training,
                                                                                                                     forward_reconstruct_coef,reverse_reconstruct_coef,generator_coef,discriminator_coef,
    ...
    reconstruct_train = reconstruct_solver.minimize(reconstruct_loss, var_list=encoder_variables+decoder_variables)
    reverse_train = reverse_solver.minimize(reverse_loss, var_list=encoder_variables)
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(epochs):
            ...
            reconstruct_losses = []
            forward_reconstruct_losses = []
            reverse_reconstruct_losses = []
            reverse_losses = []
            generator_losses = []
            ...
            for idx in range(0, imageNum, batch_size):
                ...
                image4_batch = loader(find_truth(imageName[idxes_1[idx:idx + batch_size]],imageTrue),desired_height=image_size,desired_width=image_size,value_range=(0.0, 1.0),force_grayscale=force_grayscale)
                image5_batch = loader(find_truth(imageName[idxes_3[idx:idx + batch_size]],imageTrue),desired_height=image_size,desired_width=image_size,value_range=(0.0, 1.0),force_grayscale=force_grayscale)
                feed_dict_training = {image1:image1_batch,image2:image2_batch,image3:image3_batch,image4:image4_batch,image5:image5_batch,is_training:True}

                # forward
                _,_reconstruct_loss,_forward_reconstruct_loss,_reverse_reconstruct_loss = sess.run([reconstruct_train,reconstruct_loss, forward_reconstruct_loss, reverse_reconstruct_loss],feed_dict=feed_dict_training)
                reconstruct_losses.append(_reconstruct_loss)
                forward_reconstruct_losses.append(_forward_reconstruct_loss)
                reverse_reconstruct_losses.append(_reverse_reconstruct_loss)

                # reverse
                _,_reverse_loss = sess.run([reverse_train,reverse_loss],feed_dict=feed_dict_training)
            ...
            print('epoch: %d\n - reconstruct_loss: %f, forward_reconstruct_loss: %f, reverse_reconstruct_loss: %f\n - reverse_loss: %f, generator_loss: %f, discriminator_loss: %f\n' % (epoch, get_mean(reconstruct_losses), get_mean(forward_reconstruct_losses), get_mean(reverse_reconstruct_losses), get_mean(reverse_losses), get_mean(generator_losses), get_mean(discriminator_losses)))
            ...
            image4_plot = loader(find_truth(imageName[idxes_1[0:10]],imageTrue),desired_height=image_size,desired_width=image_size,value_range=(0.0, 1.0),force_grayscale=force_grayscale)
            image5_plot = loader(find_truth(imageName[idxes_3[0:10]],imageTrue),desired_height=image_size,desired_width=image_size,value_range=(0.0, 1.0),force_grayscale=force_grayscale)
            feed_dict_not_training = {image1:image1_batch,image2:image2_batch,image3:image3_batch,image4:image4_batch,image5:image5_batch,is_training:False}
            image_class1_style3, image_class3_style1, image1_truth, image3_truth = sess.run([image1_forward_reconstruct, image2_forward_reconstruct, image1_reverse_reconstruct, image3_reverse_reconstruct],feed_dict=feed_dict_not_training)
            plot(image1_plot, image3_plot, image_class1_style3, image_class3_style1, image1_truth, image3_truth, epoch)
