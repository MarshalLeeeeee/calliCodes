...
def ae_with_gan(image1,image2,image3,image4,image5,image6,kernel,stride,class_dim,style_dim,is_training,reconstruct_coef_1,reconstruct_coef_2,reconstruct_coef_3,generator_coef,discriminator_coef,name='ae-with-gan'):
    ...
    with tf.variable_scope(name,reuse=tf.AUTO_REUSE) as scope:
        ...
        image3_pred_true = discriminator(image3,kernel,stride,is_training)
        image4_pred_true = discriminator(image4,kernel,stride,is_training)
        image5_pred_true = discriminator(image5,kernel,stride,is_training)
        image6_pred_true = discriminator(image6,kernel,stride,is_training)
        ...
        image3_pred_forward_fake = discriminator(image3_forward_reconstruct,kernel,stride,is_training)
        image4_pred_forward_fake = discriminator(image4_forward_reconstruct,kernel,stride,is_training)
        image1_style_forward_fake = discriminator(image1_style_reconstruct,kernel,stride,is_training)
        image3_style_forward_fake = discriminator(image3_style_reconstruct,kernel,stride,is_training)
        ...
        generator_loss = -generator_coef * (discriminator_forward_true_1 + discriminator_forward_true_2 + discriminator_forward_true_3 + discriminator_forward_true_4 + discriminator_style_true_1 + discriminator_style_true_3)
        discriminator_loss = -discriminator_coef * (discriminator_true_1 + discriminator_true_2 + discriminator_true_3 + discriminator_true_4 + discriminator_true_5 + discriminator_true_6 + discriminator_forward_fake_1 + discriminator_forward_fake_2 + discriminator_forward_fake_3 + discriminator_forward_fake_4 + discriminator_style_fake_1 + discriminator_style_fake_3)
